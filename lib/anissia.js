...
var AniONUtils = require('../lib/anion-utils');
	...
	return request({
		...
		if (resp.forEach) {
			resp.forEach(function(ani, ind) {
				...
				ani.time = ani.t;
				ani.ended = false;
				ani.genres = ani.g.replace(/ /g,'').split(/\/|／/).map(function (g) {
				...
				ani.genres = AniONUtils.uniqueArray(ani.genres);
	...
	return request({
		...
		if (resp.forEach) {
			resp.forEach(function(ani) {
				...
				ani.updateat = ani.d;
				var url = ani.a.trim();
				var url_p = url.parse(url);
				if (!url
					|| url.hostname.indexOf('www.test.com') > 0
					|| url.hostname.indexOf('kor.sub') > 0
				) {
					ani.url = null;
				} else {
					ani.url = 'http://' + url;
